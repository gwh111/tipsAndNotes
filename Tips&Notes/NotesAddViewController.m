...
- (void)viewDidLoad
{
    ...
    [backButton setTitle:@"Quit" forState:UIControlStateNormal];
    backButton.frame=CGRectMake(0, 20, 60, 50);
    [backButton addTarget:self action:@selector(backButtonTapped) forControlEvents:UIControlEventTouchUpInside];
    [self.view addSubview:backButton];
    
    notesTextView=[[UITextView alloc]initWithFrame:CGRectMake(10, 80, self.view.bounds.size.width-20, self.view.bounds.size.height-380)];
    ...
    UIButton *nextButtonEdit=[UIButton buttonWithType:UIButtonTypeCustom];
...
- (void)colorButton:(id)sender{
    ...
    if (button.tag==1) {
        ...
        if (notesTextView.text.length==0) {
            ...
            [[data objectForKey:@"img"]addObject:[NSString stringWithFormat:@"%d",img]];  
